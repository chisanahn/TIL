...
이렇게 현재 클래스 파일에 들어있는 위치의 경로를 읽어올 수 있는데 out 폴더에 .class 파일이 모여있는 곳이 아니라 src 폴더에 .java 파일이 모여있는 곳으로 착각해서 한참 헤맸다.

그리고 참고로 경로에 백슬래쉬를 쓰든 그냥 슬래쉬로 쓰든 상관없다. (두가지를 섞어서 써도 상관없다.)

https://stackoverflow.com/questions/1011463/creating-a-file-is-causing-problem-the-file-getpath-doesnt-seem-to-work
