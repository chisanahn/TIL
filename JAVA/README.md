...
괄호 안의 문장이 하나일 때는 괄호를 생략하고 ;를 붙이지 않고 사용할 수 있다. 단, return문만 있는 경우 다음과 같이 사용할 수 있다.

```
(int a, int b) -> a > b ? a : b
```

추론이 가능한 경우 매개변수의 타입을 생략할 수도 있다.

```java
(a, b) -> a > b ? a : b
```

매개변수의 타입을 생략할 수 있고, 매개변수가 하나 뿐이라면 괄호를 생략할 수 있다.
