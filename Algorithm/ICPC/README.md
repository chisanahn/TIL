...
> N이 1인 경우에는 1이 자기자신이라서 합이 0이 되는데 이 당시에는 이걸 생각 못했던 것 같다.
...
**참고**: map에서 key값으로 구조체를 사용할떄 값이 2개이면 구조체대신 pair 클래스를 사용하자.
    ...
    map에서 key값으로 구조체를 활용하려면 < 연산자를 override 해주어야한다.왜냐하면 map에서 key 값은 이진트리에서 탐색을 할 수 있는 조건이 되어야 하기 때문에 대소 비교가 가능해야 한다.

```c++
bool operator<(const 클래스명& 변수명) const{  //비교구문을 통해 true, false를 반환}
```
...
```c++
class DSU {    vector<int> parent;    vector<int> count;   public:    DSU(int n) : parent(n), count(n, 0) {        for (int i = 0; i <= n; i++) {            parent[i] = i;        }    }    ...}
```
...
## [2020 Seoul 인터넷 예선](https://www.acmicpc.net/category/detail/2330)
...
### [BOJ 20047 동전 옮기기](https://www.acmicpc.net/problem/20047)

> LCS 문제랑 비슷한거 같아서 LCS 풀이를 보고 다시 풀어보기로 했다.
>
> 처음 풀었을때 두 동전을 빼두고, 나머지 동전들을 비교하다가 차이가 있을때 끼워넣는 방식으로 풀었는데 차이가 없더라도 동전을 끼워넣는 경우의 수도 고려해야 할 것 같다.
>
> 99%에서 틀렸다.
>
> 따로 빼둔 동전이 맨 마지막에 오는 경우, S의 범위를 넘어가서 검사하는 부분이 생겨서 그런 것 같다.
>
> 성공. LCS 문제랑 정말 비슷하다. 아무래도 dp를 아직 잘 몰랐던 것 같다. 앞으로 dp 문제를 많이 풀어봐야겠다.



## [2019 Seoul 인터넷 예선](https://www.acmicpc.net/category/detail/2064)

2021.10.7

### BOJ 17520 Balanced String

규칙만 찾을 수 있다면 쉽게 풀 수 있는 문제

하지만 MOD가 16769023으로 int형 범위 안이지만 곱셈 과정에서 오버플로우가 발생할 수 있다는 것을 간과해서 한번 틀렸다.

### BOJ 17521 Byte Coin

간단한 구현 문제.

하지만 코인 수도 int 형 범위를 넘어갈 수 있다는 것을 간과해서 한번 틀렸다.

### BOJ 7287 Registration
...
DP 문제. 위의 4문제는 그래도 수월하게 풀었지만 이 문제는 결국 풀지 못했다. 전에 어디서 비슷한 문제를 본 것 같기도 하다.

찾아보니 DP 중에서도 가방 알고리즘으로 따로 분류된다고 한다.

https://reakwon.tistory.com/m/34

풀이를 알고보면 LCS 문제랑 그렇게 크게 차이가 나지 않는 것 같은데 역시 아직 DP에 대한 이해도가 많이 부족한 것 같다.

생각해보니 LCS의 동작과정도 아직 잘 이해하지 못해서 다시 찾아봤다.

http://melonicedlatte.com/algorithm/2018/03/15/181550.html

잊어버리고 있었는데 저번에 풀었던 동전옮기기와는 달리 LCS에서는 결과값을 저장해서 겹치는 부분을 제거해야 한다. 동전옮기기는 겹치는 부분이 없어서 그런건가?

이번에 Two Machines도 dp로 풀면 그냥 완전탐색이랑 다를게 없어서 어떻게 풀어야할지 고민했는데

그러고보니 애초에 dp는 메모이제이션이 핵심이다. 동전 옮기기는 dp가 아닌 방법으로도 풀 수 있는건가?

#### 동전옮기기 문제 분석

동전 옮기기 완전탐색 시간복잡도

> 경우의 수 O(n^2), 각 경우의 수마다 비교하는데 O(n)
>
> 총 O(n^3)
...
> 2. 문자가 다른 경우 - 코인 사용
>
> 어떻게 분석을 해야될지는 잘 모르겠다.

동전 옮기기에서 완전탐색을 할때 중복되는 부분이 어디 있을까 고민을 해보니깐 첫번째 동전을 잘못된 위치에 놓으면 두번째 동전에 상관없이 오답인 것 같다.

메모이제이션 과정이 없어서 dp로 푼건 아닌것같고 분할 정복에 더 가까운 것 같다.
...
단순히 물건을 넣고 안넣고로 케이스만 나눠서 구현했더니 시간초과.

메모이제이션을 통해 중복되는 부분을 제거해야한다.

```
int dp[pos][k];
```

솔직히 언뜻 보면 이게 중복이 발생하나? 싶다.

> 같은 무게의 물건이 여러개 있는 경우?

그리고 같은 무게지만 가치가 다른 물건이 있는 경우, 둘 중 하나만 넣었을때의 결과가 동일한 부분에 저장될것같은데
...
1 5
2 4
이렇게 입력이 주어졌을때 첫번째 물건만 넣었을때랑 두번째 물건만 넣었을때의 결과는 dp[2][8]에 저장되지 않나?
knapsack(0, 9)
	knapsack(1, 9)
		knapsack(2, 9)
			0
...
직접 해보니깐 결과값은 남아있는 물건의 개수, 남아있는 가방의 공간에 따라 결정되므로 pos와 k가 같은 경우는 동일한 결과를 갖게 되고,

가치는 다르더라도 무게가 같은 경우 물건이 있을때, 물건의 조합은 다르지만 pos와 k가 동일한 경우 중복이 발생한다.

**LCS에서도 마찬가지로 메모리제이션을 수행한다.**

뭐랄까 앞 부분과 별개로 결과가 정해지는 부분문제로 분리할 수 있으면 dp로 풀 수 있는 느낌?

#### Two Machines

재귀적으로 부분 문제로 나눠서 구현은 했는데

````c++
int f(int pos, int a, int b) {
    // base case
    if (pos == n) return a > b ? a : b;
    // A에서 작업하는 경우
    int ret = f(pos + 1, a + A[pos], b);
    // B에서 작업하는 경우
    ret = min(ret, f(pos + 1, a, b + B[pos]));
}
````

pos, a 누적 시간, b 누적 시간을 기준으로 답을 구해서 dp로 값을 저장하려면 250\*62501\*62501 크기의 3차원 배열을 사용해야 해서 메모리 범위를 초과해버린다.

-> 모두 A를 쓴다고 가정하고 B로 하나씩 바꾸는 식으로 풀어보기. 실패.
...
a_sum에 따라 dp 결과값이 달라져서 틀린 답이 나온다.

 

1. 냅색이 아닌 dp로 푸는방법 : https://newdeal123.tistory.com/5

   > dp[pos\][diff\][A or B\] 형태로 a와 b의 누적시간차이를 저장해서 푼다.

2. 냅색이 아닌 dp로 푸는 방법 2 : 

3. 냅색1 : https://alinew.tistory.com/80

   https://sbrus1213.tistory.com/3#problemL

   > A 수행시간 합이 큰 경우와 B 수행시간 합이 더 큰 경우로 나눠서 체크.
   >
   > * 배낭의 무게 = 모든 task를 A가 수행했을때 걸리는 시간
   >
   > * 물건을 넣는 행위 = B가 해당 task를 수행
   >
   > * 물건의 무게 = A[i] + B[i]
   >
   >   (A[i]도 더하는 이유는 배낭의 무게를 일정하게 유지하면서 A의 수행시간 합이 B보다 크거나 같게 하기 위해서)
   >
   > * 물건의 가치 = A[i]
   >
   >   A의 시간이 B보다 크되, A의 시간을 최대한 줄이는 것이 목표.
   >
   > 동일한 과정을 B에 대해서도 반복하고 더 작은 결과값을 출력.

1번으로 풀었을때랑 2번으로 풀었을때 메모리랑 시간 결과가 똑같다.

3. 냅색2 : https://blog.naver.com/PostView.nhn?blogId=njw1204&logNo=221669613809
...
학연산에서 모여서 예선을 치뤘는데 프린터가 가능했다면 문제를 나눠풀기도 수월하고 좋았을텐데 프린터를 할 수 없다는게 좀 많이 아쉬웠다.

난이도가 비교적 쉬웠던 2문제는 풀었지만 오답도 한번 나왔고, 먼저 푼 다른 팀에게 밀려서 교내에서는 2등으로 마무리를 했다.

1문제는 시간을 많이 들였지만 원에 포함되는 점의 개수를 구하는 부분에서 막혀서 결국 풀지 못했고,

다른 1문제는 충분히 고민할 시간이 없어서 결국 풀지 못했다.

참가에 의의를 두긴 했지만 좀 아쉬움이 남는다.

그리고 시간이 짧기도 하고 집중력에 한계도 있어서 어느 문제를 풀지 고르는 작업도 정말 중요한 것 같다. 우리 수준에서는 아직 풀지도 못할 문제에 너무 많은 시간을 들인다면 정말 낭패다.
