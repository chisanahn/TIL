...
#### 학교 알고리즘 강의를 수강하면서 깨닫게된 점

recursive relation을 이용해서 dp를 구현하면 보다 직관적으로 풀리긴 했지만 시간복잡도에 대해서 생각하기도 어려웠고 반복문으로 푸는 방법을 잘 이해하지 못했는데 이번 강의에서 반복문을 사용한 풀이를 보다보니 이에 대한 해답을 찾을 수 있었다.

시간복잡도 : 결국 dp는 메모이제이션을 통해 모든 경우의 수를 최대 한번씩 탐색할 수 있는 방법이고 따라서 시간복잡도는 메모이제이션에 사용된 배열의 크기와 같다고 할 수 있을 것 같다.

반복문으로 dp 구현하는 방법 : recursive relation을 통해 구현할 경우 부분 문제로 나누어진다. 따라서 반복문으로 구현하려면 base case부터 차례대로 구한뒤 합쳐나가는 방식으로 구현하면 된다.

[BOJ 계단 오르기](https://www.acmicpc.net/problem/2579)

간단하게 풀 수 있을 줄 알았는데 어딘가 놓친 부분이 있는지 계속 틀린다.
